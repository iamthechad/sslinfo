...
function trySSLCipher(options) {
    ...
    var socket = tls.connect(fullOptions, function() {
        deferred.resolve({protocol: options.secureProtocol, cipher: options.ciphers, enabled: true});
        socket.end();
    });
    ...
    socket.on('error', function(error) {
        ...
        if ((errorString.indexOf("socket hang up") !== -1) || (errorString.indexOf("handshake failure") !== -1)) {
            deferred.resolve({protocol: options.secureProtocol, cipher: options.ciphers, enabled: false});
        } else if (errorString.indexOf('no ciphers available') !== -1) {
            deferred.resolve({protocol: options.secureProtocol, cipher: options.ciphers, enabled: false, unsupported: true});
        } else {
